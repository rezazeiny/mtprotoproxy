...
async def handle_client(reader_clt, writer_clt):
    ...
    tcp_limit_hit = (
            ...
            (user_stats[user]["curr_connects"] > config.USER_MAX_TCP_CONNS[user]) and
            config.USER_MAX_TCP_CONNS[user] > 0
    )
    ...
    user_data_quota_hit = (
            ...
            (user_stats[user]["octets_to_client"] +
             user_stats[user]["octets_from_client"] > config.USER_DATA_QUOTA[user]) and
            config.USER_DATA_QUOTA[user] > 0
    )
...
def print_tg_info():
    ...
    print("new random secret: ", "".join(myrandom.choice("0123456789abcdef") for i in range(32)), flush=True)
